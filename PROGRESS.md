...
## 다음 액션 (2026-06-17 세션 종료 시점)
사용자가 "백엔드 연결/배포 전에 UI/UX부터 수정"하고 싶다고 결정함. 다음 세션에서 진행할 순서:
1. **UX 전체 점검 먼저 수행** — `UX_PRINCIPLES.md` 기준(직관성/일관성/가독성·접근성/유용성·반응성)으로 전체 화면을 훑고 문제점 목록을 만들어 사용자에게 먼저 보여줄 것. 바로 고치지 말고 목록 공유 → 우선순위 합의 → 수정 순서로 진행
2. 점검 후 합의된 우선순위대로 UI/UX 수정
3. (그 다음) Supabase 프로젝트 생성 후 연동 (Task #7)
4. (그 다음) 배포

호스트 등록 신청은 인앱 화면 제외 결정됨 (위 참고).
