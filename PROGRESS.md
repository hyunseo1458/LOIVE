...
## 다음 액션
1. Stitch 원본에 없는 추가 화면 포팅 (야놀자/에어비앤비 참고, 사용자 요청) — 로그인, 지역 랜딩, 위시리스트 등. 아래 "추가 화면 제안" 참고
2. Supabase 프로젝트 생성 후 연동 (Task #7)

## 추가 화면 제안 (Stitch 원본에 없던 것, 야놀자/에어비앤비 참고)
- **로그인 화면** (최우선): 지금 앱 전체가 "이미 로그인된 사용자"를 가정하고 있음 (내 정보가 고정 mock, 로그아웃 버튼도 비활성). 구글/네이버/카카오 소셜로그인 버튼 UI부터 필요
- **지역 랜딩/필터**: 홈의 지역 선택기가 라벨만 바뀌고 실제로 아무것도 필터링하지 않음. "전국 지역별 로컬 체험"이 서비스의 핵심 가치인데 지역별로 실제 걸러서 보여주는 화면이 없음
- **위시리스트(찜) 목록**: 하트 아이콘이 홈/탐색/상품상세 곳곳에 있는데 모아서 보는 화면이 없음. 내 정보의 "찜한 액티비티" 바로가기 대상
- **검색 결과**: 홈/탐색 상단 검색 아이콘이 전부 비활성 상태. 야놀자/에어비앤비 모두 검색이 핵심 진입점
- **호스트 등록 신청**: 결정된 운영 방식(호스트가 양식 제출 → 운영팀이 등록)에 맞는 신청 폼 페이지. SEO/마케팅 랜딩 역할도 가능
